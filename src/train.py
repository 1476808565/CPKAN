...
def train(args, data_info):
    ...
    for step in range(args.n_epoch):
        ...
        evalAuc.append(eval_auc)
        evalF1.append(eval_f1)
    ...
    print('evalAuc:', max(evalAuc), 'evalF1:', max(evalF1), 'testAuc:', max(testAuc), 'testF1:', max(testF1))
...
def _init_model(args, data_info):
    ...
    model = CPKAN(args, n_entity, n_relation)
    if args.use_cuda:
        model.cuda()
